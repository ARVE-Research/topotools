...
real(sp) function cellarea(lat,seconds)
  ...
  real(dp), intent(in), dimension(2) :: seconds
  ...
  resolution = seconds / 3600.d0  !convert to degrees
  ...
  area = 2.d0 * radius**2 * deltalon * cos(elevation) * sin(0.5d0 * deltalat)

  cellarea = real(area * 1.e6)
